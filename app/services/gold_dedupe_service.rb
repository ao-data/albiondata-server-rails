class GoldDedupeService
  def dedupe(data, server_id, opts)
    ...
    if REDIS[server_id].get("GOLD_RECORD_SHA256:#{sha256}").nil?
      ...
      IdentifierService.add_identifier_event(opts, server_id, 'Received on GoldDedupeService, not duplicate, sent to GoldProcessorWorker')
    else
    ...
    ActiveSupport::Notifications.instrument('metrics.gold_dedupe_service', metrics)
