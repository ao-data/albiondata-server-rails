...
module WebsiteStatsService
  SERVER_LABELS = { "east" => "Asia", "europe" => "Europe", "west" => "Americas" }.freeze

  # Unique agents (distinct client IPs) over time by server.
  class UniqueAgents
    ...
    def self.spawn_refresh_if_needed
      ...
      Thread.new do
        begin
          ...
          Rails.logger.warn("[WebsiteStatsService::UniqueAgents] background refresh failed: #{e.message}")
        ensure
    ...
    def self.fetch_data
      ...
      Rails.logger.warn("[WebsiteStatsService::UniqueAgents] #{e.message}")
      nil
    end
  ...
  # Duplicate vs non-duplicate market orders by server.
  class DuplicateItems
    FIELD_LABELS = { "duplicates" => "Duplicates", "non_duplicates" => "Non-duplicates" }.freeze

    CACHE_KEY = "duplicate_items_stats"
    REFRESH_LOCK_KEY = "duplicate_items_stats_refreshing"
    CACHE_EXPIRY = 10.minutes
    STALE_THRESHOLD = 5.minutes
    REFRESH_LOCK_EXPIRY = 2.minutes
    ...
    # Returns chart-ready data (from cache or nil). Never blocks on InfluxDB.
    # If cache is older than 5 min, triggers a background refresh to repopulate cache.
    # Format: { labels: [ISO time strings], datasets: [ { server_id, field, label, data }, ... ] }
    def self.call
      entry = Rails.cache.read(CACHE_KEY)
    ...
    def self.spawn_refresh_if_needed
      ...
      Thread.new do
        begin
          ...
          Rails.logger.warn("[WebsiteStatsService::DuplicateItems] background refresh failed: #{e.message}")
        ensure
          Rails.cache.delete(REFRESH_LOCK_KEY)
        end
      end
    ...
    def self.fetch_data
      ...
      flux = <<~FLUX.strip
        from(bucket: "#{bucket}")
          |> range(start: -24h, stop: now())
          |> filter(fn: (r) => r["_measurement"] == "market_orders")
          |> filter(fn: (r) => r["_field"] == "duplicates" or r["_field"] == "non_duplicates")
      ...
      Rails.logger.warn("[WebsiteStatsService::DuplicateItems] #{e.message}")
      nil
    end
    ...
    def self.build_chart_data(tables)
      ...
      tables.each do |table|
        ...
        server_id = table.records.first.values['server_id']&.to_s || 'unknown'
        field = table.records.first.values['_field']&.to_s || 'unknown'
        table.records.each do |rec|
          t = rec.values['_time']
          v = rec.values['_value']
          next if t.nil? || v.nil?
          ...
          time = t.is_a?(String) ? Time.zone.parse(t) : t
          time = time.utc if time.respond_to?(:utc)
          points << { time: time, server_id: server_id, field: field, value: v.to_i }
        end
      end
      ...
      labels = points.map { |p| p[:time] }.uniq.sort
      series_keys = points.map { |p| [p[:server_id], p[:field]] }.uniq.sort
      value_by = points.group_by { |p| [p[:time], p[:server_id], p[:field]] }.transform_values { |a| a.first[:value] }

      datasets = series_keys.map do |sid, field|
        region = SERVER_LABELS[sid] || sid.capitalize
        field_label = FIELD_LABELS[field] || field.gsub('_', ' ').capitalize
        {
          server_id: sid,
          field: field,
          label: "#{region} - #{field_label}",
          data: labels.map { |t| value_by[[t, sid, field]] || 0 }
        }
      end
