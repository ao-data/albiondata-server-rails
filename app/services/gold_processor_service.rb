class GoldProcessorService
  def process(data, server_id, opts)
    ...
    IdentifierService.add_identifier_event(opts, 'Received on GoldProcessorService')
    ...
    IdentifierService.add_identifier_event(opts, 'Saved to database from GoldProcessorService')
  end
end
